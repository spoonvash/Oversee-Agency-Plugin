...
class Oversee_Activator {
    ...
    public static function activate() {
        ...
        // CRITICAL: Force license re-verification on activation
        if (class_exists('Oversee_License')) {
            Oversee_License::on_plugin_activate();
        }
    ...
    public static function create_tables() {
        ...
        // Check if tickets table needs migration (old schema had ticket_id, new has id)
        $table_name = $wpdb->prefix . 'oversee_tickets';
        $table_exists = $wpdb->get_var("SHOW TABLES LIKE '{$table_name}'");
        
        if ($table_exists) {
            // Check if old ticket_id column exists
            $columns = $wpdb->get_results("SHOW COLUMNS FROM {$table_name}");
            $has_ticket_id = false;
            $has_id = false;
            foreach ($columns as $col) {
                if ($col->Field === 'ticket_id') $has_ticket_id = true;
                if ($col->Field === 'id') $has_id = true;
            }
            
            // If old schema, drop and recreate
            if ($has_ticket_id && !$has_id) {
        ...
        $sql_tickets = "CREATE TABLE {$wpdb->prefix}oversee_tickets (
            ...
            first_response_by BIGINT UNSIGNED NULL,
        ...
        $sql_agents = "CREATE TABLE {$wpdb->prefix}oversee_agents (
            ...
            calendar_embed LONGTEXT,
            show_on_live_support TINYINT(1) DEFAULT 0,
            notification_preferences LONGTEXT,
            last_seen DATETIME,
            tickets_resolved_total INT UNSIGNED DEFAULT 0,
            tickets_resolved_week INT UNSIGNED DEFAULT 0,
            tickets_resolved_month INT UNSIGNED DEFAULT 0,
            responses_total INT UNSIGNED DEFAULT 0,
            responses_week INT UNSIGNED DEFAULT 0,
            responses_month INT UNSIGNED DEFAULT 0,
            avg_first_response_seconds INT UNSIGNED DEFAULT 0,
            avg_response_time_seconds INT UNSIGNED DEFAULT 0,
            first_responses_count INT UNSIGNED DEFAULT 0,
            metrics_last_updated DATETIME NULL,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        ...
        // Sessions table
        $sql_sessions = "CREATE TABLE {$wpdb->prefix}oversee_sessions (
            id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    public static function maybe_add_agent_columns() {
        ...
        $new_columns = [
            ...
            'calendar_embed' => 'LONGTEXT NULL',
            'show_on_live_support' => 'TINYINT(1) DEFAULT 0',
            'is_online' => 'TINYINT(1) DEFAULT 0',
            'wp_user_id' => 'BIGINT UNSIGNED NULL',
            'notification_preferences' => 'LONGTEXT NULL',
            'last_seen' => 'DATETIME NULL',
            // Agent metrics columns
            'tickets_resolved_total' => 'INT UNSIGNED DEFAULT 0',
            'tickets_resolved_week' => 'INT UNSIGNED DEFAULT 0',
            'tickets_resolved_month' => 'INT UNSIGNED DEFAULT 0',
            'responses_total' => 'INT UNSIGNED DEFAULT 0',
        ...
        // Add first_response_at to tickets table if not exists
        $tickets_table = $wpdb->prefix . 'oversee_tickets';
        $ticket_columns = $wpdb->get_col("SHOW COLUMNS FROM `{$tickets_table}`");
        ...
        foreach ($ticket_new_columns as $col_name => $col_def) {
            if (!in_array($col_name, $ticket_columns)) {
                $wpdb->query("ALTER TABLE `{$tickets_table}` ADD COLUMN `{$col_name}` {$col_def}");
            }
        }
    }
}
