...
class Oversee_Migrator {
    ...
    public static function run() {
        // ALWAYS run the tickets table fix (one-time, uses its own flag)
        self::fix_tickets_table_schema();
    ...
    /**
     * Fix tickets table schema - handles old ticket_id vs new id column
     */
    private static function fix_tickets_table_schema() {
        global $wpdb;
        
        // Check if already fixed
        if (get_option('oversee_tickets_schema_v2', false)) {
            return;
        }
        ...
        if (!$table_exists) {
            // Table doesn't exist, create_tables will handle it
            update_option('oversee_tickets_schema_v2', true);
            return;
        }
